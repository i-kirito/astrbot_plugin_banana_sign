...
class BananaSign(Star):
    ...
    def __init__(self, context: Context, config: AstrBotConfig):
        ...
        # ========== 积分系统初始化 ==========
        # 积分数据目录
        self.sign_data_dir = os.path.join(os.getcwd(), "data", "astrbot_plugin_banana_sign")
        if not os.path.exists(self.sign_data_dir):
            os.makedirs(self.sign_data_dir, exist_ok=True)
        self.sign_data_file = os.path.join(self.sign_data_dir, "user_data.json")
        self.user_data = self._load_sign_data()

        # 积分配置
        sign_config = self.conf.get("sign_config", {})
        self.daily_reward = sign_config.get("daily_reward", 1)
        ...
        # 数据目录
        data_dir = StarTools.get_data_dir("astrbot_plugin_banana_sign")
    ...
    def _load_sign_data(self) -> Dict[str, Any]:
        """加载用户签到数据"""
        try:
            if os.path.exists(self.sign_data_file):
    ...
    async def set_banana_cmd(self, event: AstrMessageEvent, target_id: str = "", amount: str = ""):
        ...
        for comp in event.get_messages():
            if isinstance(comp, Comp.At) and comp.qq:
                at_target = str(comp.qq)
                break

        # 优先使用 @用户，否则使用参数
        final_target_id = at_target or target_id

        # 如果使用了 @，amount 可能在 target_id 位置
        final_amount = amount if amount else (target_id if at_target else "")
